...
while read ofid bircid age sex lunaid visitdate rest; do
 ...
 for rn in 1 2 3 4; do
  oftaskdir=task001_run$(printf %03d $rn)
  ## files
  ...
  # if we only need to ditch the scorer, we could just run sed on the score file
  #paste <(sed 1d $scorefile) <(egrep "^$runtype" txt/listOrders.txt) | sed "s/	/ /g" >> "$behavdata"
  # but the score file is also missing some trials
  paste <(
    perl -slane '
      BEGIN{our $i=1;sub pl {print join("\t",($i,-1,-1));$i++}}
      next if $.<=1;
      pl() while($i<$F[0]);
      print; $i++;
      END{pl() while $i<25}' $taskdir/$lunaid/$visitdate/Scored/txt/$lunaid.$visitdate.$rn.manual.txt
    ) <(egrep "^$runtype" txt/listOrders.txt) | sed "s/	/ /g" >> "$behavdata"
